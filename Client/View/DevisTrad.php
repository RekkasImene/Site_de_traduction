<?php
class DevisTrad{
  public function devisTrad_page(){ ?>
  <!DOCTYPE html>
...
<div class="ecran" style="background-color: #e3f2fd!important;margin-top: 150px;">
  <h1 style="font-weight: bold; font-size:48px;color:black;text-align: center;">Choisissez le traducteur que vous voulez: </h1>
<?php

                    try{
                        $ct=new TraducteurControler();
                        $qtm=$ct->getDevisTradControler();
                        foreach ($qtm as $rowm) {
                            ?>
                    <a class="btn btn-primary" href="choixtradRooter.php?Email=<?php echo $rowm['email']; ?>"> Choisir ce traducteur </a>
                  ...
                  <div class="infos">
                    <p style="font-weight: bold;"> Type de traduction métrisé: </p> <p> <?php echo $rowm["type"];?></p>
                  </div>
                   <div class="infos">
                    <p style="font-weight: bold;"> Assermenté: </p><p> <?php echo $rowm["assermente"];?></p>
                  </div>
                   <div class="infos">
                    <p style="font-weight: bold;"> Note (/10): </p> <p><?php echo $rowm["note"];?></p>
                  </div>
        ...
        ?>
<!--<br>
<input type="checkbox" name="tradDefault" id="myCheck" ><h1 style="margin-top: 50px;">Choisir un traducteur par défaut</h1>
